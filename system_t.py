...
class System_T:
    ...
    def selective_collect(self, budget, k, batch_size, lr, epochs, cost_func, Lambda, num_iter, slice_desc, strategy="one-shot", show_figure=False):
        ...
        while True: 
            ...
            if self.budget < 5:
                ...
                print("Loss: %.5f (%.5f), Average EER: %.5f, Max EER: %.5f\n" % tuple(self.show_performance()))
                break
            else:
                self.train_on_subsets(num_k, num_iter)
        ...
        for i in range(num_iter):
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for k in range(len(num_k)):
                    batch_jobs.append(executor.submit(self.training, num_k, k))
    ...
    def train_after_collect_data(self, num_examples, num_iter):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            batch_jobs = []
            for i in range(num_iter):
                batch_jobs.append(executor.submit(self.training, num_k, i))
    ...
    def training(self, num_k, k):
        ...
        network = Model(self.train[0][:num_k[k]], self.train[1][:num_k[k]], self.val[0], self.val[1], self.val_data_dict, 
                      self.batch_size, epochs=self.epochs, lr = self.lr, num_class = self.num_class)
        loss_dict, slice_num, process_num = network.train(k)
    ...
    def show_performance(self):
        """ Average validation loss(standard deviation), average equalized error rate(Avg. EER), maximum equalized error rate (Max. EER) """
