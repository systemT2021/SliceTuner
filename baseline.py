...
class Baseline:
    ...
    def training(self, process_num):
        ...
        network = Model(self.train[0], self.train[1], self.val[0], self.val[1], self.val_data_dict, 
                      self.batch_size, epochs=self.epochs, lr = self.lr, num_class = self.num_class)
        loss_dict, slice_num, process_num = network.train(process_num)
    ...
    def train_after_collect_data(self, num_examples, num_iter):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i in range(num_iter):
                batch_jobs.append(executor.submit(self.training, i))
